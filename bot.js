...
client.on("message", msg => {
    ...
    const embedlul = new Discord.RichEmbed()
      .setColor(0x00AE86)
      .setDescription(":crown: " + msg.author + " Reklam Koruması Aktif Edildi!:crown:")

const embedlulz = new Discord.RichEmbed()
    .setTitle("Sunucunda " + msg.author.tag + "REKLAM YAPIYOR!")
      .setColor(0x00AE86)
      .setDescription(" sb?warn <kişi> komutu ile onu uyarabilir ya da sb?kick <kişi> veya sb?ban <kişi> komutlarını kullanarak onu sunucudan uzaklaştırabilirsin!")
...
.then(channel =>
                   channel.setParent(message.guild.channels.find(channel => channel.name === "Bot Kullanımı")));
message.guild.createChannel(`Yapımcım: SunaySu Aydın`, 'voice')
.then(channel =>
                   channel.setParent(message.guild.channels.find(channel => channel.name === "Bot Kullanımı")));
...
if (message.content === `<@522005758401839116>`) {
 message.channel.sendEmbed(new Discord.RichEmbed()
                           ...
                           .addField('Hello;', 'Botun sahibi : 𝐌𝐨𝐨𝐧𝐒𝐡𝐢𝐧𝐞#1528 dir gerekli bilgiler sb?bilgi !')
                           .addField('Prefixim:','sb?')
...
client.on("message", msg => {
    const uyarıembed = new Discord.RichEmbed()
      .setColor(0x00AE86)
      .setDescription("Heey   + msg.author +  ---> Lütfen Reklam Yapma. !! ")

const dmembed = new Discord.RichEmbed()
  .setTitle("Sunucunda " + msg.author.tag + " reklam yapıyor!")
  .setColor(0x00AE86)
  .setDescription("Sunucunda Reklam Yapıyor sb?uyar komutu ile kişiyi uyara bilirsin fakat `mod-log` kanalın olması lazım.")
  .addField("Kullanıcının mesajı:", "**" + msg.content + "**")
...
client.on('guildMemberAdd', member => {
  ...
  const channel = member.guild.channels.find('name', 'SunaySu Aydın');
  if (!channel) return;
 const embed = new Discord.RichEmbed()
 .setColor('RANDOM')
 ...
 .setFooter('SunaySu Aydın', client.user.avatarURL)
 .setTimestamp()
 channel.send(embed);
});
...
client.ayarlar = {
        "sahip": ["741321235400622180"],  //kendi IDnızı yazınız
};
