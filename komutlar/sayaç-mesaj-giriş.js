...
exports.run = async (client, message, args) => {
	if(!args[0]) {
		const embed = new Discord.RichEmbed()
			.setDescription(` **Lütfen Sayacı Ayarlamam İçin Bir Sayı Belirt! (örnek: e-sayaç 5000)**`)
	...
	if(isNaN(args[0])) {
		const embed = new Discord.RichEmbed()
			.setDescription(`Lütfen Sayaçı Ayarlamam İçin Bir Sayı Bilirt (örnek: sb?sayaç 5000`)
			.setColor(ayarlar.renk)
		message.channel.send({embed})
		return
	}
